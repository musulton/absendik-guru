...
import { fetchGradeMonthlyTrend } from "@/lib/recap-monthly-trend";
...
export function GradeMonthlyTrendChart({
  workspaceId,
  classId,
  month,
  subjectName,
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    void fetchGradeMonthlyTrend(workspaceId, classId, month, subjectName).then(
      (next) => {
        if (!cancelled) {
          setPoints(next);
          setLoading(false);
  ...
  return (
    <RecapLineChart
      title={t("recap.chartGradesTrend")}
      points={points}
